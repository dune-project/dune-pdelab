...
void driver(std::string basis_type, std::string part_unity_type) {
  ...
  if (configuration.get<int>("nev") >= 0) {
    ...
    Dune::VTKWriter<GM::LeafGridView> vtkwriter(grid->leafGridView());
    FS::DGF xdgf(fs.getGFS(),x);
    vtkwriter.addVertexData(std::make_shared<FS::VTKF>(xdgf,"x_h"));
    vtkwriter.write("amg",Dune::VTK::appendedraw);

    if (Dune::MPIHelper::getCollectiveCommunication().rank() == 0) std::cout << "Solver finished: F=" << timer_full.elapsed() << std::endl;
