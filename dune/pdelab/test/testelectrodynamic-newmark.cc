...
#include "../localoperator/electrodynamic-newmark.hh"
                       ...
                       const std::string filename, Probe &probe)
{
  ...
  *xcur = 0.0;
  ...
  EpsType eps(gv,eps0);
  typedef Dune::PDELab::ElectrodynamicNewmark<EpsType,MuType,V> LOP; 
  LOP lop(eps, mu, beta, Delta_t, integrationOrder);
               ...
               LPF &lpf, ELPF &elpf, double &error, double &mean_h)
{
  ...
  std::cout << "electrodynamic-newmark level " << level << std::endl;
  // time step
          ...
          double conv_limit, std::string name = "")
{
  ...
  std::cout << std::endl
            << "Testing ElectrodynamicNewmark problem with EdgeS03D and " << name << std::endl;

  std::string filename = "electrodynamic-newmark-" + name;
  ...
  std::cout << "electrodynamic-newmark total convergence: "
  ...
  if(total_convergence < conv_limit) {
    std::cout << "Error: electrodynamic-newmark total convergence < " << conv_limit << std::endl;
    result = 1;
  }
}
...
//   }
  ...
  try{
    ...
    Dune::SmartPointer<PointPF>
      pointPF(new PointPF("electrodynamic-newmark-probe", probe_location_fv));

    typedef ResonatorVTKGridProbeFactory<double> VTKOutput;
    Dune::SmartPointer<VTKOutput>
      vtkOutput(new VTKOutput("electrodynamic-newmark"));

    typedef ResonatorGlobalErrorGridProbeFactory GlobalError;
    Dune::SmartPointer<GlobalError>
      globalError(new GlobalError(quadrature_order, "electrodynamic-newmark-globalerror", "electrodynamic-newmark-globalevolution"));

    typedef ResonatorL2ErrorGridProbeFactory L2Error;
    Dune::SmartPointer<L2Error>
      l2Error(new L2Error("electrodynamic-newmark-l2error", quadrature_order));

    typedef ResonatorL2ErrorEvolutionGridProbeFactory L2Evolution;
    Dune::SmartPointer<L2Evolution>
      l2Evolution(new L2Evolution("electrodynamic-newmark-l2evolution", quadrature_order));

    typedef ElectricEnergyGridProbeFactory EnergyEvolution;
    Dune::SmartPointer<EnergyEvolution>
      energyEvolution(new EnergyEvolution("electrodynamic-newmark-electricenergy-evolution", quadrature_order));
