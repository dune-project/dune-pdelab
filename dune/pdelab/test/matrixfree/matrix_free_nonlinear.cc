...
int main(int argc, char** argv)
{
  try{
   ...
   // Create constraints map
    ...
    // palpo
    GridOperator::Domain applyVector(gridFunctionSpace);
    applyVector = 1.0;
    GridOperator::Range resultVector(gridFunctionSpace);
    gridOperator.jacobian_apply(coefficientVector, applyVector, resultVector);
    using Dune::PDELab::Backend::native;
    // Dune::printvector(std::cout, native(resultVector), "palpo result", "row");
