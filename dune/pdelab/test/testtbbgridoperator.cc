...
bool poisson (const GV& gv, const FEM& fem, std::string filename, int q)
{
  ...
  // make grid operator
  typedef Dune::PDELab::PerElementLockManager<GV, std::mutex> LockManager;
  typedef Dune::RangedPartitioning<GV, 0> Partitioning;
  typedef Dune::PDELab::TBBGridOperator<Partitioning,GFS,GFS,LOP,
                                        MBE,
                                        double,double,double,
                                        LockManager,
                                        C,C> GridOperator;
  GridOperator gridoperator(gfs,cg,gfs,cg,lop,
                            std::make_shared<LockManager>(gv),mbe);
  auto partitions = std::max(std::thread::hardware_concurrency(), 1u);
...
int main(int argc, char** argv)
{
  try{
...

    {
      ...
      // solve problem
      bool good = poisson<GV,FEM,Dune::PDELab::ConformingDirichletConstraints>(gv,fem,"tbbgridoperator_yasp_Q1_2d",2);
...

    {
      ...
      // solve problem
      bool good = poisson<GV,FEM,Dune::PDELab::ConformingDirichletConstraints>(gv,fem,"tbbgridoperator_yasp_Q2_2d",2);
      accumulate_result(result, good);
    }

    {
      ...
      // solve problem
      bool good = poisson<GV,FEM,Dune::PDELab::ConformingDirichletConstraints>(gv,fem,"tbbgridoperator_yasp_Q1_3d",2);
      accumulate_result(result, good);
    }

    {
      ...
      // solve problem
      bool good = poisson<GV,FEM,Dune::PDELab::ConformingDirichletConstraints>(gv,fem,"tbbgridoperator_yasp_Q2_3d",2);
      accumulate_result(result, good);
    }
