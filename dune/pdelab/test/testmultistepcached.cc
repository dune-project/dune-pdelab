...
class CachePolicy :
  public Dune::PDELab::MultiStepCachePolicy<Step, Time>
{
public:
...
void wave (const GV& gv, const FEM& fem, typename GV::ctype dt,
           typename GV::ctype c, int steps, std::string filename)
{
  // constants and types
  ...
  typedef CachePolicy<int,DF> Policy;
...
// #else
  ...
  typedef Dune::PDELab::DiscreteGridFunction<GFS,V> DGF;
  {
    ...
    vtkwriter.write(-dt,Dune::VTK::base64);
    vtkwriter.clear();
  }
  {
    ...
    vtkwriter.addVertexData
      (new Dune::PDELab::VTKGridFunctionAdapter<DGF>(dgf,"solution"));
    vtkwriter.write(0,Dune::VTK::base64);
    vtkwriter.clear();
  }
  ...
  for(int step = 1; step <= steps; ++step) {
    ...
    vtkwriter.addVertexData
      (new Dune::PDELab::VTKGridFunctionAdapter<DGF>(dgf,"solution"));
    vtkwriter.write(time,Dune::VTK::base64);
    vtkwriter.clear();
  }
}
  ...
  try{
    ...
    // YaspGrid P1 1D test
    {
      ...
      wave<GV,FEM,Dune::PDELab::NonoverlappingConformingDirichletConstraints,2>
        (gv,fem,1.0/(1<<8), 1.0, 1<<9 ,"testmultistepcached_yasp_P1_1d");
    }
