  ...
  namespace PDELab{
    ...
    private:
      ...
      void assemble(LocalAssemblerEngine & assembler_engine, Direction direction) const
      {
        ...
        // Traverse grid view
        for (const auto& element : elements(entity_set,assembler_engine.partition(),direction))
          {
            // Compute unique id
                        ...
                        if (require_uv_skeleton || require_v_skeleton)
                          {
                            ...
                            bool visit_face = ids > idn
                              ...
                              or (assembler_engine.partition() == Partitions::interiorBorder
                                  and outside_element.partitionType() != InteriorEntity);
