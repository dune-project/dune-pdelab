...
namespace Dune{
  namespace PDELab{
    ...
    */
    template<typename OSLA, bool doPreStage>
    ...
    public:
      ...
      */
      ...
      void setLocalResidualEngine(ResidualEngine & residual_engine_)
      {
      ...
      void onBindLFSUV(const EG & eg, const LFSU & lfsu, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onBindLFSUV(eg,lfsu,lfsv);
      ...
      void onBindLFSV(const EG & eg, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onBindLFSV(eg,lfsv);
      ...
      void onBindLFSUVInside(const IG & ig, const LFSU & lfsu, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onBindLFSUVInside(ig,lfsu,lfsv);
      ...
      {
        if(doPreStage) prestage_engine->onBindLFSUVOutside(ig,lfsus,lfsvs,lfsun,lfsvn);
      ...
      void onBindLFSVInside(const IG & ig, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onBindLFSVInside(ig,lfsv);
      ...
      void onBindLFSVOutside(const IG & ig,
                             ...
                             const LFSV_N & lfsvn)
      {
        if(doPreStage) prestage_engine->onBindLFSVOutside(ig,lfsvs,lfsvn);
      ...
      void onUnbindLFSV(const EG & eg, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onUnbindLFSV(eg,lfsv);
      ...
      void onUnbindLFSVInside(const IG & ig, const LFSV & lfsv){
        if(doPreStage) prestage_engine->onUnbindLFSVInside(ig,lfsv);
      ...
      void onUnbindLFSVOutside(const IG & ig,
                               ...
                               const LFSV_N & lfsvn)
      {
        if(doPreStage) prestage_engine->onUnbindLFSVOutside(ig,lfsvs,lfsvn);
      ...
      {
        if(doPreStage) prestage_engine->onUnbindLFSUV(eg,lfsu,lfsv);
      ...
      {
        if(doPreStage) prestage_engine->onUnbindLFSUVInside(ig,lfsu,lfsv);
      ...
      {
        if(doPreStage) prestage_engine->onUnbindLFSUVOutside(ig,lfsu_s,lfsv_s,lfsu_n,lfsv_n);
      ...
      void loadCoefficientsLFSUInside(const LFSU & lfsu_s){
        if(doPreStage) prestage_engine->loadCoefficientsLFSUInside(lfsu_s);
      ...
      void loadCoefficientsLFSUOutside(const LFSU & lfsu_n){
        if(doPreStage) prestage_engine->loadCoefficientsLFSUOutside(lfsu_n);
      ...
      {
        if(doPreStage) prestage_engine->loadCoefficientsLFSUCoupling(lfsu_c);
      ...
      void preAssembly(){
        if(doPreStage) prestage_engine->preAssembly();
         residual_engine->preAssembly();
        jacobian_engine->preAssembly();
      }

      void postAssembly(){
        if(doPreStage) prestage_engine->postAssembly();
      ...
      {
        const bool abort_a = doPreStage ? prestage_engine->assembleCell(eg) : true;
      ...
      {
        if(doPreStage) prestage_engine->assembleUVVolume(eg,lfsu,lfsv);
        residual_engine->setWeights();
        residual_engine->assembleUVVolume(eg,lfsu,lfsv);
        jacobian_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleVVolume(eg,lfsv);
        residual_engine->setWeights();
      ...
      void assembleUVSkeleton(const IG & ig, const LFSU_S & lfsu_s, const LFSV_S & lfsv_s,
                              const LFSU_N & lfsu_n, const LFSV_N & lfsv_n)
      {
        if(doPreStage) prestage_engine->assembleUVSkeleton(ig,lfsu_s,lfsv_s,lfsu_n,lfsv_n);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleVSkeleton(ig,lfsv_s,lfsv_n);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleUVBoundary(ig,lfsu_s,lfsv_s);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleVBoundary(ig,lfsv_s);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleVEnrichedCoupling(ig,lfsv_s,lfsv_n,lfsv_coupling);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleUVVolumePostSkeleton(eg,lfsu,lfsv);
        residual_engine->setWeights();
      ...
      {
        if(doPreStage) prestage_engine->assembleVVolumePostSkeleton(eg,lfsv);
        residual_engine->setWeights();
