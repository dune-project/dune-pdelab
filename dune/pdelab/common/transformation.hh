  ...
  namespace PDELab {
    namespace TypeTree {
      ...
      struct TransformTree
      {
        ...
        {
          return TransformTree<SourceTree,Transformation,typename SourceTree::tag>::transform(s,t);
        }
      ...
      struct TransformTree<S,T,PowerNodeTag>
      {
        ...
        static transformed_type transform(const S& s, T& t)
        {
          ...
          // transform node
          return TransformNode<S,T>::template transform<
            typename TransformTree<typename S::ChildType,T,typename S::ChildType::NodeTag>::transformed_type
            >(s,t,children);
        }
        ...
        static transformed_storage_type transform_storage(shared_ptr<const S> sp, T& t)
        {
          ...
          return TransformNode<S,T>::template transform_storage<
            typename TransformTree<typename S::ChildType,T,typename S::ChildType::NodeTag>::transformed_type
            >(sp,t,children);
        }
      ...
      struct transform_nonvariadic_composite_node;
      ...
      template<typename S, typename T, typename C0, typename C1, typename C2, typename C3, typename C4, typename C5, typename C6, typename C7, typename C8>
      struct transform_nonvariadic_composite_node<S,tuple<C0,C1,C2,C3,C4,C5,C6,C7,C8>,T>
      {
        ...
        static transformed_storage_type transform_storage(shared_ptr<const S> sp, T& t)
        {
          return TransformTree<S,T>::template transform_storage<
            typename TransformTree<C0,T,typename C0::NodeTag>::transformed_type,
            typename TransformTree<C1,T,typename C1::NodeTag>::transformed_type,
            typename TransformTree<C2,T,typename C2::NodeTag>::transformed_type,
            typename TransformTree<C3,T,typename C3::NodeTag>::transformed_type,
      ...
      {
        typedef typename transform_nonvariadic_composite_node<S,typename S::Children,T>::transformed_type transformed_type;
        typedef typename transform_nonvariadic_composite_node<S,typename S::Children,T>::transformed_storage_type transformed_storage_type;

        static transformed_type transform(const S& s, T& t)
        {
          return transform_nonvariadic_compositenode<S,typename S::Children,T>::transform(s,t);
        }
        ...
        static transformed_storage_type transform_storage(shared_ptr<const S> sp, T& t)
        {
          return transform_nonvariadic_compositenode<S,typename S::Children,T>::transform_storage(sp,t);
        }
      ...
      struct transform_variadic_composite_node<S,tuple<C...>,T>
      {
        ...
        transform(const S& s, T& t, const C2&... children)
        {
          return TransformNode<S,T>::template transform<
            typename transformation<C,T,typename C::NodeTag>::transformed_type...
            >(s,t,TransformTree<C,T,typename C::NodeTag>::transform_storage(children)...);
        }
        ...
        template<typename... C2>
        static typename enable_if<sizeof...(C) == sizeof...(C2), transformed_type>::type
        transform(const S& s, const T& t, const C2&... children)
        {
          return TransformNode<S,T>::template transform<
            typename TransformTree<C,T,typename C::NodeTag>::transformed_type...
            >(s,t,TransformTree<C,T,typename C::NodeTag>::transform_storage(children)...);
        }
        ...
        transform_storage(shared_ptr<const S> sp, T& t, const C2&... children)
        {
          return TransformNode<S,T>::template transform_storage<
            typename TransformTree<C,T,typename C::NodeTag>::transformed_type...
            >(sp,TransformTree<C,T,typename C::NodeTag>::transform_storage(children)...);
        }

        template<typename... C2>
        static typename enable_if<(sizeof...(C) > sizeof...(C2)), transformed_storage_type>::type
        transform_storage(shared_ptr<const S> sp, const C2&... children)
        {
          return transform_storage(sp,children...,sp->template childStorage<sizeof...(C2)>());
        }
      };
      ...
      // CompositeNode::ChildType member and forwards to the helper struct
      template<typename S, typename T>
      struct transformation<S,T,VariadicCompositeNodeTag>
      {
      ...
      // handle empty nodes
      template<typename T>
      struct TransformTree<EmptyChild,T,EmptyNodeTag>
      {
        // get transformed type from specification
        typedef EmptyChild transformed_type;
        typedef shared_ptr<EmptyChild> transformed_storage_type;

        // delegate instance transformation to per-node specification
        static transformed_type transform(const EmptyChild& s, const T& t)
        {
          DUNE_THROW(NotImplemented,"this should never get called!");
        }

        static transformed_storage_type transform_storage(shared_ptr<const EmptyChild> en, const T& t)
        {
