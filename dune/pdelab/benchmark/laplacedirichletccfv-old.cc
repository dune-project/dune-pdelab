  ...
  for (std::size_t run = 0; run < runs; ++run)
    {
      ...
      // this is only here to generate output consistent with
      ...
      typedef Dune::PDELab::GridOperator<
        GFS,GFS,LO,
        Dune::PDELab::ISTLVectorBackend<1>::MatrixBackend,
        RF,RF,RF> GO;
      GO go(gfs,gfs,lo);
      ...
      if (solve)
        {
          ...
          Dune::SeqILU0<ISTLM,ISTLV,ISTLV> ilu0(m,1.0);
...
int main(int argc, char** argv)
{
  try{
    ...
    if (argc == 1)
      {
        std::cerr << "no parameter file passed, defaulting to laplacedirichletccfv-old.ini..." << std::endl;
        Dune::ParameterTreeParser::readINITree("laplacedirichletccfv-old.ini",params);
      }
    else if (argc == 2)
      {
    ...
    // 2D - standard mapper
    if (params.hasSub("Yasp_2D_general") && params.get<bool>("Yasp_2D_general.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_2D_general");
        ...
        test(grid.leafView(),mapper,name("Yasp",grid,"general",solve),solve,runs);
      }

    // 2D - restricted mapper
    if (params.hasSub("Yasp_2D_restricted") && params.get<bool>("Yasp_2D_restricted.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_2D_restricted");
        ...
        typedef Dune::PDELab::GridFunctionRestrictedMapper Mapper;
        Mapper mapper;

        test(grid.leafView(),mapper,name("Yasp",grid,"restricted",solve),solve,runs);
      }

    // 2D - static mapper
    if (params.hasSub("Yasp_2D_static") && params.get<bool>("Yasp_2D_static.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_2D_static");
        ...
        typedef Dune::PDELab::SimpleGridFunctionStaticSize Mapper;
        Mapper mapper;

        test(grid.leafView(),mapper,name("Yasp",grid,"static",solve),solve,runs);
      }


    // 3D - standard mapper
    if (params.hasSub("Yasp_3D_general") && params.get<bool>("Yasp_3D_general.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_3D_general");
    ...
    // 3D - restricted mapper
    if (params.hasSub("Yasp_3D_restricted") && params.get<bool>("Yasp_3D_restricted.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_3D_restricted");
    ...
    // 3D - static mapper
    if (params.hasSub("Yasp_3D_static") && params.get<bool>("Yasp_3D_static.enabled"))
      {
        const Dune::ParameterTree& lp = params.sub("Yasp_3D_static");
